...
@Component({
  selector: 'dispensation-formulas-action-cmp',
  templateUrl: './dispensation-formulas-action.component.html'
})
export class DispensationFormulasActionComponent extends BaseModel implements OnInit {
  ...
  private saveAudit(audit_state_id) {
    if (this.__product.length > 0) {
      ...
      this.helperService.POST(`/api/dispensation-formulas`, this.model).subscribe(rs => {
        const res = rs.json();
        if (res.store) {
  ...
  private updateAudit(audit_state_id) {
    if (this.__product.length > 0) {
      ...
      this.model.details = this.__product;
      this.loaderService.display(true);
      this.helperService.PUT(`/api/dispensation-formulas/${this.numId}`, this.model).subscribe(rs => {
        const res = rs.json();
        if (res.update) {
  ...
  private getDataById(): void {
    this.loaderService.display(true);
    this.helperService.GET(`/api/dispensation-formulas/${this.numId}`).subscribe(rs => {
      const res = rs.json();
  ...
  private cancelAudit() {
    this.helperService.PUT(`/api/dispensation-formulas/cancel/${this.numId}`, {}).subscribe(rs => {
      const res = rs.json();
      if (res.cancel) {
  ...
  private auditar() {
    ...
    if (product.length > 0) {
      ...
      this.helperService.PUT(`/api/dispensation-formulas/auditar/${this.numId}`, this.model).subscribe(rs => {
        const res = rs.json();
        if (res.auditada) {
  ...
  private finalizar(audit_state_id) {
    this.helperService.PUT(`/api/dispensation-formulas/finalize/${this.numId}/${audit_state_id}`, this.model).subscribe(rs => {
      const res = rs.json();
      if (res.finalize) {
